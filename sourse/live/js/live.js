...
var vm = new Vue({
    ...
    data: {
        ...
        textLiveData: [], //文字直播数据
        textLiveStatus:false, //是否有文字直播
        //playList:[], //播放列表
    },
    created() {
        this.getWx() //获取微信配置
        this.getWxModel() //判断是否在小程序内
    },
    mounted() {
        ...
        //this.backTopShow()
        //this.isVision()
        document.getElementById('myAudio') != null ? this.audioInit() : '' //初始化音频
    ...
    computed: {
        ...
        playControlPrev() {
            return this.isPlayIndex == 0 ? 'dark-grey' : ''
        },
        playControlNext() {
            return this.isPlayIndex == this.playerList.length - 1 ? 'dark-grey' : ''
        }
    },
    filters: {
        msgTime(timestamp) {
    ...
    methods: {
        ...
        getAudioPlayState() {
            ...
            if (this.audioEndedStorage != null) {
                let audioEndedStorage = this.audioEndedStorage
                this.playerList.map((item, index) => {
                    let id = "" + this.config.live_room_id + '_' + item.mid
                    if (audioEndedStorage[id] != undefined) {
                        let arr = item
                        arr.ended = true
        ...
        audioEnded(e) {
            ...
            let setArr = this.playerList[this.isPlayIndex]
            setArr.ended = true
            Vue.set(this.playerList, this.isPlayIndex, setArr)
        ...
        audioControlPrev() {
            ...
            this.$refs.myplayer[this.isPlayIndex].getParentAudioState(false)
            this.$refs.myplayer[this.isPlayIndex].getParentAudioShowState(false)
            this.audioPlayState = true
            this.isPlayIndex = parseInt(this.isPlayIndex) - 1
            this.isPlaySrc = this.playerList[this.isPlayIndex].audio_src
            this.useAudioControl
            let _ = this
            setTimeout(() => {
                _.audioRef.play()
            }, 50)
        ...
        audioControlNext() {
            ...
            this.audioPlayState = true
            this.isPlayIndex = parseInt(this.isPlayIndex) + 1
            this.isNextPlaySrc = this.isPlayIndex != this.playerList.length - 1 ? this.playerList[this.isPlayIndex + 1].audio_src : this.isPlaySrc
            this.isPlaySrc = this.playerList[this.isPlayIndex].audio_src
            let _ = this
            setTimeout(() => {
                _.audioRef.play()
        ...
        change(index) {
            let _ = this
            _.$refs.myplayer[_.isPlayIndex].getParentAudioShowState(false)
            _.isPlayIndex = index
            _.isNextPlaySrc = _.isPlayIndex != _.playerList.length - 1 ? _.playerList[_.isPlayIndex + 1].audio_src : _.isPlaySrc
            _.isPlaySrc = _.playerList[_.isPlayIndex].audio_src
            setTimeout(() => {
                _.audioRef.play()
            }, 50)
        },
        ...
        //组件里回调 - 定位当前音频块 - 减慢循环
        animateScroll(number = 0, time) {
            if (!time) {
                document.querySelector('.talk-container').scrollTop = number;
                return number;
            }
            let spacingTime = 20; // 设置循环的间隔时间  值越小消耗性能越高
            let spacingInex = time / spacingTime; // 计算循环的次数
            //let nowTop = document.body.scrollTop + document.documentElement.scrollTop; // 获取当前滚动条位置
            let nowTop = document.querySelector('.talk-container').scrollTop;
            ...
            //     if (spacingInex > 0) {
            //         spacingInex--;
            //         that.animateScroll(nowTop += everTop);
            //     }
            //     setTimeout(arguments.callee,spacingTime);
            // },spacingTime)
            that.scrollTimer = setInterval(function() {
                if (spacingInex > 0) {
                    ...
                    clearInterval(that.scrollTimer); // 清除计时器
                }
            }, spacingTime);
        ...
        backToTop() {
            ...
            offset = Math.min(offset, document.querySelector('.talk-container').scrollTop)
            document.querySelector('.talk-container').scrollTop = offset;
        },
        /*  ======================== 音频功能 end  ========================  */
        ...
        toggleTextLiveItem(e) {
            ...
            this.textLiveData[index].opened = !this.textLiveData[index].opened
            this.textLiveData = Object.assign({}, this.textLiveData);
        },
        //定位是否提醒用户有新消息
        scrollHandler() {
            if (document.querySelector('.talk-container') == null) {
                return
            }
        ...
        startOverLive(data) {
            ...
            for (var i = 0; i < this.wallList.length; i++) {
                if (this.wallList[i].media_type == 'audio') {
                    ...
                    let arr = this.wallList[i]
                    arr.ended = false
                    this.playerList.push(arr)
                }
            }
            //设置默认播放
            this.isPlaySrc = this.playerList.length >= 1 ? this.playerList[this.isPlayIndex].audio_src : null
            this.isNextPlaySrc = this.isPlayIndex != this.playerList.length - 1 && this.playerList.length > 1 ?
                this.playerList[this.isPlayIndex + 1].audio_src : this.isPlaySrc
            ...
            if (this.userInfo.user_type == 'student' && this.status.user_live_status == 3) {
                this.danmu()
            }
            this.getPlayerList();
            this.createClient()
        ...
        getListData(type) {
            ...
            post(url, { id: that.config.live_room_id }, '加载中', function(res) {
                ...
                if (res.code) {
                    ...
                    that.textLiveData = liveData.live_text ? liveData.live_text : []
                    ...
                    that.textLiveData.length > 0 ? (that.filterTextLiveData(),that.textLiveStatus = true) : [] //添加属性到文字直播里
                } else {
        ...
        sendMsg() {
            if (this.editText === '' || this.editText === null) {
                console.log('不能为空');
                this.userInfo.user_type == 'student' ? this.$Message.info('发送内容不能为空'):this.$Message.info('发送内容不能为空')
                return false;
            }
            this.openEdit(); //关闭编辑
            ...
            if ((this.userInfo.user_type === 'teacher' || this.userInfo.user_type === 'team') && this.swiperImg.length == 0) {
        ...
        openEdit() {
            ...
            //     document.querySelector('.edit-msg').scrollIntoView(true);
            // },1500)
        },
        //打开/关闭  评论/问题 浮框
        toggleComment() {
