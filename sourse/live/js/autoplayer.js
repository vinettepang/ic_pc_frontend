var player = {
  ...
  <div class="ic-player" :class="{ player, 'player_active': audioState }" :style="{ width: playerWidth }">
    ...
    <div class="progress-bar" ref="progressBar">
      <div class="bar-inner" v-if="audioShowState">
  ...
  //     <i class="nc-icon nc-check-circle-07-2 icon-checked"  v-show="playState === 2"></i>
    ...
    data() {
        return {
            ...
            audioShowState:false, //音频显示状态
            bufferState:true, //缓冲状态
    ...
    computed: {
        playIcon() {
            return this.audioState && !this.bufferState ?  'ivu-icon ivu-icon-pause' : !this.audioState && !this.bufferState ? 'spin-icon-load ivu-icon ivu-icon-load-c' : 'ivu-icon ivu-icon-play'
            //return this.audioState ?  'ivu-icon ivu-icon-pause' : 'ivu-icon ivu-icon-play'
        },
        playStateIcon(){
            return this.audioState ? 'nc-icon nc-wifi' : this.ended ? 'nc-icon nc-check-circle-07-2' : ''
        }
    },
    created() {
        this.touch = {}
    ...
    watch: {
        ...
        audioState(state) {
            if (state) {
                this.playState = 1
                this.audioShowState = true
            } else {
                if (this.playState != 2) {
                    this.playState = 0
                } else {
                    this.playState = this.ended ? 2 : 0
                }
            }
        },
        percent(newPercent) {
    ...
    methods: {
        ...
        getParentAudioState(state) {
            ...
            if (state) {
                this._events.audioswiper[0](this.pptId)
            }
            //this.audioState ? '' : this.audioShowState = false
        },
        getParentAudioShowState(state){
            this.audioShowState = state
        },
        getParentAudioEnd(state){
            this.playState = 2
            this.audioShowState = false
        },
